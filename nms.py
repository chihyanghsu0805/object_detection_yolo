...
def py_nms(boxes, scores, max_boxes=50, iou_thresh=0.5):
  ...
  Arguments: boxes: shape of [-1, 4], the value of '-1' means that dont know the exact number of boxes
             scores: shape of [-1,]
...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.5, iou_thresh=0.5):
  ...
  params:
      ...
      score_thresh: if [ highest class probability score < score_threshold] then get rid of the corresponding box
  ...
  for i in range(num_classes):
    ...
    nms_indices = tf.image.non_max_suppression(boxes=filter_boxes, scores=filter_score, max_output_size=max_boxes, iou_threshold=iou_thresh, name='nms_indices')
